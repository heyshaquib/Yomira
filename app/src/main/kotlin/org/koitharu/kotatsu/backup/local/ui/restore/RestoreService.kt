...
class RestoreService : BaseBackupRestoreService() {
	...
	@Inject
	lateinit var migrationUseCase: YomiraMigrationUseCase
	...
	override suspend fun IntentJobContext.processIntent(intent: Intent) {
		...
		applicationContext.powerManager.withPartialWakeLock(TAG) {
			...
			// If the restored backup came from another Yomira fork (it carries built-in source
			...
			runCatching {
				if (migrationUseCase.scan().isNotEmpty()) {
					YomiraMigrationService.start(applicationContext)
				}
