...
/**
 * Predefined mapping of legacy Yomira parser source names (the `MangaParserSource` enum
 * constant strings stored in Yomira backups, e.g. `MANGADEX`, `WEBTOONS_EN`) to their
 * equivalent installed Mihon source. The table is baked into `res/raw/Yomira_source_map.json`,
 * auto-generated by matching the Yomira-parsers(-redo) source list against the keiyoushi
 ...
 * Yomira forks ship different parser forks, so the source name in a backup may not exist in
 * Yomira's own `MangaParserSource`. The table is the single source of truth and is trivially
...
class KotatsuSourceMap @Inject constructor(
	...
	/** Resolve a legacy Yomira source name (e.g. `MANGADEX`) to its Mihon target. */
	...
	private fun parse(): Tables = try {
		...
		for ((enumName, entry) in file.entries) {
			...
			// Many Yomira enums can map to one Mihon id (language variants); first wins.
...
/** The installed Mihon source a legacy Yomira source maps to. */
